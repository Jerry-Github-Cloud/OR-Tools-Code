...
class Graph:
    def __init__(self, args):
        ...
        self.op_m_edge_idx = []
        self.m_op_edge_idx = []
        self.m_m_edge_idx = self.fully_connect(0, args.machine_num - 1)
    ...
    def update_feature(self, jobs, machines, current_time, max_process_time):
        ...
        for job in jobs:
            for op in job.operations:
                ...
                if status == PROCESSED:
                    ...
                    #feat.append((op.finish_time - current_time) / self.args.max_process_time)
                else:
                    feat.append(0)
                # process time
                feat.append(op.process_time / max_process_time)
                #feat.append((op.process_time) / self.args.max_process_time)
                # remaining operations
                feat.append((len(job.operations) - op.op_id) / self.args.op_num)
        ...
        for m in machines:
            ...
            if status == AVAILABLE:
                ...
                #feat.append((m.avai_time() - current_time) / self.args.max_process_time)
